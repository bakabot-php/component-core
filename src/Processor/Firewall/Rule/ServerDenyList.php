...
final class ServerDenyList extends FirewallRule
{
    ...
    public function detailedMessage(Message $message): ?string
    {
        ...
        return sprintf(
            "Ignoring message in %s channel [%s] (server: [%s]) - is on server's deny list",
    ...
    public function handle(Message $message): Promise
    {
        return call(function () use ($message) {
            ...
            if ($settings->denyList->contains($channel)) {
                return $this->fail();
            }
    ...
    {
        return self::PREFIX_SERVER . 'deny-list';
    }
}
